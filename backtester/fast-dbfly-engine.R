...
# same for strikes
PickByStrike = function(my.strikes, my.value) {
  which.min(abs(my.value - my.strikes))
}
...
# FindDBF: find the strikes of 60-40-20 trade by delta: long 60/20 short 2x 40
# inputs:
...
FindDBF = function(my.df, my.price) {
  ...
  if (nrow(my.df) < 10) {
    ...
    # if you have two expirations, pick the max for now
    my.df = subset(my.df, cal.dte == max(cal.dte))
    # deltas are negative for puts
    my.df.puts = subset(my.df, Call.Put == "P")
    
    atm.iv = my.df.puts[PickByDelta(my.df.puts[,6], my.price),9]
    far.put.strike = my.price-(my.price*atm.iv*(35/365)^0.5)
    far.call.strike = my.price+(my.price*atm.iv*(35/365)^0.5)
    
    my.df.puts[PickByDelta(my.df.puts[,6],  my.price),1] =  1 # col 16 is Delta, 1 is Ex.Pos.
    my.df.puts[PickByDelta(my.df.puts[,6],  far.put.strike),1] =  1
    middle.strike = mean(subset(my.df.puts, Existing.Posn. == 1)$Strike.Price)
    my.df.puts[PickByDelta(my.df.puts[,6], middle.strike),1] = -2
    
    # deltas are positive for calls
    my.df.calls = subset(my.df, Call.Put == "C")
    my.df.calls[PickByDelta(my.df.calls[,6],  my.price),1] =  1 # col 16 is Delta, 1 is Ex.Pos.
    my.df.calls[PickByDelta(my.df.calls[,6],  far.call.strike),1] =  1 # col 21 is prob ITM
    middle.strike = mean(subset(my.df.calls, Existing.Posn. == 1)$Strike.Price)
    my.df.calls[PickByDelta(my.df.calls[,6], middle.strike),1] = -2
    
    # Put them back together
    my.df = rbind.data.frame(my.df.calls, my.df.puts)
...
# calculate Theta 
FindTheta = function(my.df) {
  sum(my.df[,1] * my.df[,19])
}

# return the strike price of the far call in a call bfly
FarCall = function(trades) {
  strike = max(trades[trades$Call.Put == "C",6])
}

# return the strike price of the far put in a put bfly
FarPut = function(trades) {
  strike = min(trades[trades$Call.Put == "P",6])
}
...
ShouldExit = function(my.df, my.highs, my.lows, my.date) {
  ...
  if (FindDTR(my.df) > kDTRThresh)
    return(TRUE)
  else if (bizdays(as.Date(my.date), as.Date(my.df[1,24]), my.cal) <= 2) 
    return(TRUE)
  else if (as.numeric(my.highs[my.date]) >= FarCall(my.df)) 
    return(TRUE)
  else if (as.numeric(my.lows[my.date]) <= FarPut(my.df))
    return(TRUE)
  else if (FloatingProfit(my.df) > 0.1*InitialCredit(my.df))
    return(TRUE)
  else if (FindTheta(my.df) < 0)
    return(TRUE)
  else
    return(FALSE)
}
...
ExitReason = function(my.df, my.highs, my.lows, my.date) {
  ...
  if (FindDTR(my.df) > kDTRThresh) 
    ...
    return(paste("2: 2 biz DTE"))
  else if (as.numeric(my.highs[my.date]) >= FarCall(my.df)) 
    return(paste("3: Far call hit"))
  else if (as.numeric(my.lows[my.date]) <= FarPut(my.df))
    return(paste("4: Far put hit"))
  else if (FloatingProfit(my.df) > 0.1*InitialCredit(my.df))
    return(paste("5: 10% profit hit"))
  else if (FindTheta(my.df) < 0)
    return(paste("6: Negative Theta"))
  else 
...
ShouldEnter = function(my.trade.list, my.quote, my.price) {
  ...
  would.trade.exp = unique(FindDBF(my.quote, my.price)$my.exp.date)
...
for (i in 88:(length(my.data)-1)) { 
  ...
  last.price = as.numeric(Cl(get(my.sym)[names(my.data)[i]]))
  potential.bfly = FindDBF(my.data[[i]], last.price)
  if (global.mode == "1TPX" && 
      as.numeric(oisuf.values[names(my.data)[i]]) > kOisufThresh && 
      ShouldEnter(open.trades, my.data[[i]], last.price) && 
      !is.null(potential.bfly) && 
      nrow(potential.bfly) == 6 && 
      FindDTR(potential.bfly) < kDTRThresh) {
