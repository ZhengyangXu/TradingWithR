...
# To set some constraints, lets say 
#   my investment in C must be less than sum of A, B.  
#   Also, investment in A cannot be more than twice of B.  
#   Finally, at least 10% of investment in each stock.
...
model = make.lp(0, 3)
# define the objective function: for minimize, use negative
set.objfn(model, c(-0.05, -0.04, -0.06)) # = x1*5% + x2*4% + x3*6%
# add constraints
add.constraint(model, c(1,  1,  1), "=", 1) #  x1 + x2 + x3 = 1
add.constraint(model, c(1,  1, -1), ">", 0) #  x1 + x2 - x3 > 0
add.constraint(model, c(1, -2,  0), "<", 0) #  x1 - 2*x2    < 0
# set upper and lower bounds #  10% < x1, x2, x3 < 100%
...
#get.constraints(model) # 1 0 0 see above, it's the last arg to add.constraint
