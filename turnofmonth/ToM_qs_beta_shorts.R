...
tickers      = c('T', 'VZ', 'PFE')
short_tickers = c('MSFT', 'GE', 'JNJ')
# Need short_tickers, long_tickers for pair trade setup
...
for (tick in tickers) {
  ...
  m_bandh = as.integer(initShares) * Ad(to.monthly(get(tick)[paste(paste(begin_year,"02",sep='-'),end_year,sep='::')]))
...
for (tick in short_tickers) {
   ...
   tick = gsub("[][^]", "", tick)
   initPrice  = Ad(first(to.monthly(get(tick))[paste(paste(begin_year,"01",sep='-'),end_year,sep='::')]))
   initShares = floor(initCash / length(short_tickers) / initPrice)
   initBal    = initPrice * initShares
   ...
   for (y in begin_year:end_year) {
      for (i in 1:12) {
         if (i == 12) {
            ...
            tmp_g      = rbind(first(last(get(tick)[paste(y, i, sep="-")],"5 days")),
                               last(first(get(tick)[paste((y+1), 1, sep="-")],"3 days")))
            tmp_g_delt = as.numeric(Ad(tmp_g)[1])/as.numeric(Ad(tmp_g)[2])-1
            tmp_g_x    = reclass(tmp_g_delt, Ad(tmp_g)[2])
            final_g    = rbind(final_g, tmp_g_x)
         }
         else if (is.null(final_g)) {
            tmp_g      = rbind(first(last(get(tick)[paste(y, i, sep="-")],"5 days")),
                               last(first(get(tick)[paste(y, (i+1), sep="-")],"3 days")))
